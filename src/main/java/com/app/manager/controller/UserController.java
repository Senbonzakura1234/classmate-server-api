...
public class UserController {
    ...
    public String register(@Validated @ModelAttribute RegisterModel registerModel,
        ...
        if(bindingResult.hasErrors()){
            ...
            var basicSubscription = subscriptionService.getBasicSubscription();
            if(basicRole.isEmpty() || basicSubscription.isEmpty()) return "views/authen/register";
            List<UserRole> roleList = new ArrayList<>();
            var user = new User();
            user.setUsername(registerModel.getUsername());
            user.setEmail(registerModel.getEmail());
            user.setPassword(encoder.encode(registerModel.getPassword()));
            user.setSubscriptionId(basicSubscription.get().getId());
